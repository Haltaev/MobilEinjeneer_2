...
import kotlinx.android.synthetic.main.fragment_return_from_warehouse.*
...
class ReturnFromWarehouseFragment : BaseFragment() {
    ...
    lateinit var returnToWarehouseConfirmViewModel: ReturnFromWarehouseViewModel
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        button_confirm.setOnClickListener {
            ...
            if (items.size > 1) {
                ...
                returnToWarehouseConfirmViewModel.putItemsToMyWarehouse(movingItems)
            } else {
                val item = MovingAcceptRequest()
                item.apply {
                    mcId = System.currentTimeMillis()
                    destinationId = items[0].targetWarehouseId
                    destinationSide = 0
                    quantity = counter
                    skuId = items[0].skuId
                }
                items[0].movingId?.let {
                    movingItems.add(Pair(it, item))
                }
                returnToWarehouseConfirmViewModel.putItemsToMyWarehouse(movingItems)
...
//            (activity as ActionWithItemActivity).finish()
    ...
    private fun observeViewModel(viewModel: ReturnFromWarehouseViewModel) {
        viewModel.apply {
