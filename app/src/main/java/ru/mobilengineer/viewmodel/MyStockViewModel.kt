...
class MyStockViewModel @Inject constructor(var context: Context) : ViewModel() {
    @Inject
    ...
    var dataLiveData: MutableLiveData<ArrayList<MyStockResponse>> = MutableLiveData()
    var deleteLiveData: MutableLiveData<Any> = MutableLiveData()
    var errorLiveData: MutableLiveData<Int?> = MutableLiveData()


    fun getMyStock(filterMode: String) {
        viewModelScope.launch {

            try {
                val response = apiService.getMyStock(filterMode)
    ...
    fun cancelTransfer(movingId: String) {
        viewModelScope.launch {
