...
import ru.mobilengineer.data.api.model.response.MovingCreateResponse
import java.net.UnknownHostException
import javax.inject.Inject

class TransferToEngineerConfirmViewModel @Inject constructor(var context: Context) : ViewModel() {
    @Inject
    ...
    var dataLiveData: MutableLiveData<ArrayList<MovingCreateResponse>> = MutableLiveData()
    var errorLiveData: MutableLiveData<Int?> = MutableLiveData()


    fun transferToEngineer(actionWithItem: ArrayList<MovingCreateRequest>) {
        viewModelScope.launch {

            try {
                val response = apiService.movingCreate(actionWithItem)
